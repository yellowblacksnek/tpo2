...
public class TanIntegrationTests {
    ...
    public static Collection<Object[]> tanTestData() {
        return Arrays.asList(new Object[][] {
                ...
                {1.047, 1.731},{-1.047, -1.731},{PI/2, Double.NaN},
                {-PI/2, Double.NaN},{2.094, -1.734},{-2.094, 1.734},
                {2.356, -1.000},{-2.356, 1.000},{2.618, -0.577},
                {-2.618, 0.577},{3.142, 0.000},{-3.142, -0.000},
                {3.665, 0.577},{-3.665, -0.577},{3.927, 1.000},
                {-3.927, -1.000},{4.189, 1.733},{-4.189, -1.733},
                {3*PI/2, Double.NaN},{-3*PI/2, Double.NaN},{5.236, -1.732},
    ...
    @DisplayName("integration test tan")
    ...
    public void testValues(double x, double y) {
        ...
        Cos cosSpy = spy(new Cos(EPSILON));
        Tan tan = new Tan(cosSpy, sinSpy);

        assertEquals(y, tan.apply(x), DELTA);
        if(Math.abs(Math.cos(x)) >= 1.0e-15f) verify(sinSpy).apply(x);
        verify(cosSpy).apply(x);
    }
}
